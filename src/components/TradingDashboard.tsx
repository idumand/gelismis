...
export const TradingDashboard: React.FC<TradingDashboardProps> = ({
  ...
  const [engineMetric, setEngineMetric] = useState<any>(null);

  useEffect(() => {
    let mounted = true;
    const load = async () => {
      try {
        const res = await fetch('/api/v1/deepdata');
        const data = await res.json();
        const metric = data?.metrics?.[selectedPair];
        if (mounted) setEngineMetric(metric || null);
      } catch {}
    };
    load();
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="lg:col-span-3 space-y-4">
          ...
          {engineMetric && (
            <div className="bg-[#151921] border border-[#1e232f] rounded-xl p-3 shadow-lg">
              <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">ARGOS V8 Karar Motoru</span>
                  <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                    engineMetric.adaptiveDecision === 'ENTER_NOW' ? 'bg-emerald-500/20 text-emerald-300' :
                    engineMetric.adaptiveDecision === 'ENTER_BETTER' ? 'bg-cyan-500/20 text-cyan-300' :
                    engineMetric.adaptiveDecision === 'WATCH' ? 'bg-amber-500/20 text-amber-300' :
                    'bg-slate-800 text-slate-400'
                  }`}>
                    {engineMetric.adaptiveDecision || 'WARMUP'}
                  </span>
                </div>
                <span className="text-[10px] text-slate-500 font-mono">{engineMetric.regime || '—'}</span>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2">
                {[
                  ['Skor', `${Number(engineMetric.adaptiveEntryScore || 0).toFixed(0)}`],
                  ['Olasılık', `%${Number(engineMetric.forecastHitProbabilityPct || 0).toFixed(1)}`],
                  ['B/E', `%${Number(engineMetric.adaptiveBreakEvenProbabilityPct || 0).toFixed(1)}`],
                  ['EV', `$${Number(engineMetric.forecastExpectedValueUSD || 0).toFixed(2)}`],
                  ['Hedef', `%${Number(engineMetric.adaptiveTargetPct || 0).toFixed(2)}`],
                  ['Stop', `%${Number(engineMetric.adaptiveStopLossPct || 0).toFixed(2)}`],
                  ['Urgency', `${Number(engineMetric.adaptiveUrgency || 0).toFixed(0)}`],
                  ['Erozyon', `${Number(engineMetric.erosionRiskScore || 0).toFixed(0)}`],
                ].map(([label, value]) => (
                  <div key={label} className="bg-[#0b0e14] rounded-lg border border-slate-800 px-2.5 py-2">
                    <div className="text-[9px] uppercase tracking-wider text-slate-500">{label}</div>
                    <div className="mt-0.5 text-xs sm:text-sm font-mono font-bold text-white">{value}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {engineMetric && (
            <div className="bg-[#151921] border border-[#1e232f] rounded-xl p-3 shadow-lg">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400">Long / Short Para Akışı — Capital Battle</span>
                <span className="text-[10px] text-slate-500">Karar: {engineMetric.selectedSide === 'long' ? 'LONG' : 'SHORT'}</span>
              </div>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                {[
                  ['LONG Baskınlık', `%${Number(engineMetric.longDominanceScore || 0).toFixed(0)}`],
                  ['LONG Direnç', `%${Number(engineMetric.longResistanceScore || 0).toFixed(0)}`],
                  ['LONG Net Kâr', `$${Number(engineMetric.longExpectedNetProfitUSD || 0).toFixed(2)}`],
                  ['LONG EV', `$${Number(engineMetric.longExpectedValueUSD || 0).toFixed(2)}`],
                  ['SHORT Baskınlık', `%${Number(engineMetric.shortDominanceScore || 0).toFixed(0)}`],
                  ['SHORT Direnç', `%${Number(engineMetric.shortResistanceScore || 0).toFixed(0)}`],
                  ['SHORT Net Kâr', `$${Number(engineMetric.shortExpectedNetProfitUSD || 0).toFixed(2)}`],
                  ['SHORT EV', `$${Number(engineMetric.shortExpectedValueUSD || 0).toFixed(2)}`],
                ].map(([label, value]) => (
                  <div key={label} className="bg-[#0b0e14] rounded-lg border border-slate-800 px-2.5 py-2">
                    <div className="text-[9px] uppercase tracking-wider text-slate-500">{label}</div>
                    <div className="mt-0.5 text-xs sm:text-sm font-mono font-bold text-white">{value}</div>
              ...
              <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] font-mono">
                <div className="bg-[#0b0e14] rounded px-2 py-1.5 text-slate-300">Hedef Olasılığı: <b>%{Number(engineMetric.selectedHitProbabilityPct || 0).toFixed(1)}</b></div>
                <div className="bg-[#0b0e14] rounded px-2 py-1.5 text-slate-300">Seçili EV: <b>${Number(engineMetric.selectedExpectedValueUSD || 0).toFixed(2)}</b></div>
                <div className="bg-[#0b0e14] rounded px-2 py-1.5 text-slate-300">Risk/Getiri: <b>{Number(engineMetric.selectedRiskReward || 0).toFixed(2)}</b></div>
                <div className="bg-[#0b0e14] rounded px-2 py-1.5 text-slate-300">Karar Güveni: <b>%{Number(engineMetric.selectedConfidencePct || 0).toFixed(0)}</b></div>
              </div>
            </div>
          )}
                  ...
                  return (
                    <tr key={t.id} className="hover:bg-slate-800/30 transition">
                      ...
                      <td className="py-3 px-4 text-slate-400">
                        ...
                        {t.model_forecast_expected_value_usd !== undefined && (
                          <div className={`font-semibold ${t.model_forecast_expected_value_usd >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>
                            Beklenen Değer: {t.model_forecast_expected_value_usd >= 0 ? '+' : ''}${t.model_forecast_expected_value_usd.toFixed(2)} · Hedef olasılığı: %{(t.model_forecast_hit_probability_pct || 0).toFixed(0)}
                          </div>
                        )}
                      </td>
                      <td className="py-3 px-4">
