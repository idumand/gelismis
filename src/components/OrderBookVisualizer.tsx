...
export const OrderBookVisualizer: React.FC<OrderBookVisualizerProps> = ({ pair = 'BTC/USDT' }) => {
  ...
  }, [metrics?.aggressiveLongPct, metrics?.aggressiveShortPct, metrics?.aggressiveMoneyDeltaUSD,
      metrics?.pressureDurabilityScore, metrics?.opposingResistanceScore, metrics?.erosionRiskScore,
      metrics?.capitalEfficiencyScore, metrics?.liveTargetPrice, metrics?.expectedTargetPrice]);
