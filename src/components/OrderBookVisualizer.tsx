...
export const OrderBookVisualizer: React.FC<OrderBookVisualizerProps> = ({ pair = 'BTC/USDT' }) => {
  ...
  useEffect(() => {
    ...
    const load=async()=>{
      try{
        const r=await fetch(`/api/v1/orderbook?symbol=${encodeURIComponent(pair)}`);
        if(!r.ok) return;
        const d=await r.json();
        if(active){ setData(d); setIsLiveConnected(true); }
      }catch{ if(active)setIsLiveConnected(false); }
    };
    load();
  ...
  return (
    <div className="bg-[#151921] border border-[#1e232f] rounded-xl flex flex-col overflow-hidden">
      <div className="px-4 py-3 border-b border-[#1e232f] bg-[#0f1218] flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-semibold text-sm text-slate-200">
            Binance Futures Derin Analiz & Para Akışı
          </h3>
      ...
      <div className="px-4 pt-3 grid grid-cols-2 md:grid-cols-5 gap-2 text-[10px] font-mono">
        <div className="bg-[#0b0e14] rounded-lg p-2"><span className="text-slate-500">LONG BASKI</span><div className="text-emerald-300 font-bold">%{Math.round(longPressurePct)}</div></div>
        <div className="bg-[#0b0e14] rounded-lg p-2"><span className="text-slate-500">SHORT BASKI</span><div className="text-rose-300 font-bold">%{Math.round(shortPressurePct)}</div></div>
        <div className="bg-[#0b0e14] rounded-lg p-2"><span className="text-slate-500">HEDEF 1x</span><div className="text-amber-300 font-bold">+%{Number(predictedProfitPct).toFixed(2)}</div></div>
        <div className="bg-[#0b0e14] rounded-lg p-2"><span className="text-slate-500">DESTEK</span><div className="text-slate-200 font-bold">{supportPrice?supportPrice.toPrecision(7):'—'}</div></div>
        <div className="bg-[#0b0e14] rounded-lg p-2"><span className="text-slate-500">DİRENÇ</span><div className="text-slate-200 font-bold">{resistancePrice?resistancePrice.toPrecision(7):'—'}</div></div>
      </div>
