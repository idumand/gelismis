...
export const OrderBookVisualizer: React.FC<OrderBookVisualizerProps> = ({ pair = 'BTC/USDT' }) => {
  ...
  useEffect(() => {
    ...
    const cleanSym = pair.replace('/', '').toLowerCase();
    ...
    // 1. Establish direct WebSocket connection to Binance SPOT stream
    const connectWebSocket = () => {
      try {
        const wsUrl = `wss://stream.binance.com:9443/stream?streams=${cleanSym}@depth20@100ms/${cleanSym}@aggTrade/${cleanSym}@ticker`;
        ...
        ws.onmessage = (event) => {
          ...
          try {
            ...
            if (stream && stream.endsWith('@aggTrade')) {
              // Real-time Binance Spot Aggregated Trade
              const price = parseFloat(streamData.p);
              const qty = parseFloat(streamData.q);
              const isBuyerMaker = Boolean(streamData.m);
              tradesBufferRef.current.push({
                price,
                qty,
                isBuyerMaker,
                time: streamData.T || Date.now()
              });
            } else if (stream && stream.endsWith('@depth20@100ms')) {
              // Real-time 100ms Binance Spot Order Book Depth
              const bids = (streamData.bids || []).map((b: any) => [parseFloat(b[0]), parseFloat(b[1])]);
              const asks = (streamData.asks || []).map((a: any) => [parseFloat(a[0]), parseFloat(a[1])]);
              ...
              const spreadPct = midPrice > 0 ? (bestAsk - bestBid) / midPrice : 0.0001;

              const bidVol = bids.reduce((acc: number, b: any) => acc + b[1], 0);
              const askVol = asks.reduce((acc: number, a: any) => acc + a[1], 0);
              const totalVol = bidVol + askVol;

              const obi = totalVol > 0 ? (bidVol - askVol) / totalVol : 0;
              const microPrice = totalVol > 0 ? ((bestBid * askVol) + (bestAsk * bidVol)) / totalVol : midPrice;

              const flow = computeFlowMetrics(midPrice);
              ...
              const newMetrics = {
                ...
                VWAP: flow.vwap,
                SpreadPct: spreadPct,
                deepScore: Math.round((obi * 50) + ((flow.takerBuyRatio - 0.5) * 100)),
                takerBuyRatio: flow.takerBuyRatio,
                netInflowUSD: flow.netInflowUSD,
                totalTrades: flow.totalTradesCount
              };
    ...
    // 2. Immediate Binance SPOT REST Fallback & Recent Trades Seed
    const seedInitialData = async () => {
      try {
        const symbolUpper = cleanSym.toUpperCase();
        const [depthRes, tradesRes] = await Promise.all([
          fetch(`https://api.binance.com/api/v3/depth?symbol=${symbolUpper}&limit=20`),
          fetch(`https://api.binance.com/api/v3/trades?symbol=${symbolUpper}&limit=50`)
        ]);

        if (depthRes.ok && isMounted) {
          ...
          const midPrice = (bestBid + bestAsk) / 2 || bestBid || 1;
          const bidVol = bids.reduce((acc: number, b: any) => acc + b[1], 0);
          const askVol = asks.reduce((acc: number, a: any) => acc + a[1], 0);
          const totalVol = bidVol + askVol;
          const obi = totalVol > 0 ? (bidVol - askVol) / totalVol : 0;
          ...
          setData({
            ...
            metrics: {
              ...
              currentPrice: midPrice,
              VWAP: flow.vwap,
              SpreadPct: bestAsk > 0 ? (bestAsk - bestBid) / midPrice : 0.0001,
              deepScore: Math.round(obi * 50),
              takerBuyRatio: flow.takerBuyRatio,
              netInflowUSD: flow.netInflowUSD
            }
          });
    ...
    // Periodic safety polling to ensure continuous tick even on mobile webview pauses
      ...
      <div className="px-4 py-3 border-b border-[#1e232f] bg-[#0f1218] flex items-center justify-between">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="font-semibold text-sm text-slate-200">
            Binance Spot Derin Analiz & Para Girişi Monitörü (Spot Tahta)
          </h3>
          <span className={`inline-flex items-center space-x-1 px-1.5 py-0.5 rounded text-[10px] font-mono font-bold ${
            ...
            <span>{isLiveConnected ? 'BINANCE SPOT CANLI (100ms)' : 'GÜNCELLENİYOR'}</span>
          </span>
        </div>
        <div className="flex items-center space-x-2">
