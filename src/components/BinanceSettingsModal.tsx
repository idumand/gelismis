...
export const BinanceSettingsModal: React.FC<BinanceSettingsModalProps> = ({ isOpen, onClose, onSave, serverIp = 'Tespit ediliyor...' }) => {
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await onSave(apiKey, secretKey, environment);
    } finally {
      setIsSaving(false);
      onClose();
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#151921] border border-[#2a3142] rounded-xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="bg-blue-500/10 border border-blue-500/30 p-3 rounded-lg space-y-2">
            ...
            <select
              ...
              className="w-full bg-[#0b0e14] border border-blue-500/30 text-slate-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:border-blue-400 transition-all"
            >
              <option value="demo">🧪 DEMO (Binance Demo Trading Testnet)</option>
              <option value="live">⚠️ LIVE (Gerçek Para - Dikkatli!)</option>
            </select>
            <p className="text-[10px] text-blue-300/80 leading-snug">
              {environment === 'demo' 
                ? 'DEMO seçildi: Binance Demo Trading API anahtarlarını kullanın. Test işlemleri için güvenlidir.' 
                : 'LIVE seçildi: Gerçek Binance hesabınızın canlı API anahtarlarını kullanacaksınız. DIKKATIYLE!'}
            </p>
          </div>
          ...
          <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-lg flex items-start gap-3">
            ...
            <p className="text-xs text-amber-300/90 leading-relaxed">
              API anahtarlarınız doğrudan Node.js arka plan motoruna iletilecek ve botun işlem yapabilmesini sağlayacaktır. Lütfen çekim (withdrawal) yetkisi OLMAYAN bir anahtar kullanın.
            </p>
          </div>
        ...
        <div className="flex items-center justify-end gap-3 p-4 border-t border-[#1e232f] bg-[#0b0e14]/50">
          ...
          <button
            onClick={handleSave}
            disabled={!apiKey || !secretKey || !environment || isSaving}
            title={!apiKey || !secretKey ? 'API Key ve Secret Key zorunlu' : !environment ? 'Ortam seçimi zorunlu' : ''}
          ...
          >
            {isSaving ? `${environment === 'live' ? 'LIVE\'e ' : 'DEMO\'ya '} Bağlanıyor...` : `${environment === 'live' ? 'LIVE\'e' : 'DEMO\'ya'} Bağlan`}
          </button>
        </div>
      </div>
    </div>
