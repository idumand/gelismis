...
export function App() {
  ...
  // Market data is served by the backend so TESTNET/LIVE always uses the selected Futures environment.
  useEffect(() => {
    let mounted = true;
    const refresh = async () => {
      try {
        const res = await fetch('/api/v1/live-tickers');
        const data = await res.json();
        if (!mounted || !Array.isArray(data?.tickers)) return;
        const tickMap = new Map<string, any>();
        data.tickers.forEach((item: any) => {
          const symbol = item.symbol?.replace('/', '');
          if (symbol && Number(item.price) > 0) {
            tickMap.set(symbol, item);
            livePricesRef.current.set(symbol, Number(item.price));
          }
        });
        setMarkets(prev => prev.map(m => {
          const t = tickMap.get(m.symbol.replace('/', ''));
          if (!t) return m;
          return { ...m, price: Number(t.price), change_24h_pct: Number(t.change_24h_pct || 0), volume_24h_usdt: Number(t.volume_24h_usdt || 0) };
        }));
        setTrades(prev => prev.map(t => {
          if (!t.is_open) return t;
          const rate = livePricesRef.current.get(t.pair.replace('/', ''));
          if (!rate) return t;
          const pnlUSD = t.type === 'short' ? (t.open_rate - rate) * t.amount : (rate - t.open_rate) * t.amount;
          const margin = (t.open_rate * t.amount) / (t.leverage || 1);
          const roe = margin > 0 ? (pnlUSD / margin) * 100 : 0;
          return { ...t, current_rate: rate, profit_pct: Number(roe.toFixed(2)), profit_abs: Number(pnlUSD.toFixed(2)), profit_ratio: roe / 100 };
        }));
      } catch {}
    };
    refresh();
  ...
  useEffect(() => {
    ...
    const fetchKlines = async () => {
      try {
        ...
        // Keep existing real candles; never invent market data.
      }
    };
