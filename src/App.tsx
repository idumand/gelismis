...
import { AlgorithmBrain } from './components/AlgorithmBrain';
...
export function App() {
  ...
  const [selectedStrategy, setSelectedStrategy] = useState('OrderFlow_Quant_Brain');
  ...
  // Market data is served by the Render backend so the browser never mixes Spot and Futures feeds.
  useEffect(() => {
    let active = true;
    const processTickData = (items:any[]) => {
      ...
      items.forEach((item:any)=>{
        ...
        const price=Number(item.price||item.lastPrice||item.c||0);
        if(symbol&&price>0){ tickMap.set(symbol.replace('/',''),item); livePricesRef.current.set(symbol.replace('/',''),price); }
      });
      setMarkets(prev=>{
        const bySymbol=new Map(prev.map(m=>[m.symbol,m]));
        items.forEach((t:any)=>{
          const symbol=String(t.symbol||'').includes('/')?String(t.symbol):`${String(t.symbol||'').replace('USDT','')}/USDT`;
          if(!symbol||symbol==='/USDT')return;
          const old=bySymbol.get(symbol);
          bySymbol.set(symbol, old ? {...old,
            price:Number(t.price||t.lastPrice||t.c||old.price),
            change_24h_pct:Number(t.change_24h_pct??t.priceChangePercent??old.change_24h_pct),
            volume_24h_usdt:Number(t.volume_24h_usdt??t.quoteVolume??old.volume_24h_usdt),
            signal:(Number(t.deepScore||0)>8?'BUY':Number(t.deepScore||0)<-8?'SELL':'NEUTRAL') as any
          } : {
            symbol,base:symbol.split('/')[0],quote:'USDT',
            price:Number(t.price||t.lastPrice||t.c||0),
            change_24h_pct:Number(t.change_24h_pct??t.priceChangePercent??0),
            volume_24h_usdt:Number(t.volume_24h_usdt??t.quoteVolume??0),
            high_24h:0,low_24h:0,in_whitelist:false,in_blacklist:false,
            signal:(Number(t.deepScore||0)>8?'BUY':Number(t.deepScore||0)<-8?'SELL':'NEUTRAL') as any
          });
        });
        return Array.from(bySymbol.values()).sort((x,y)=>y.volume_24h_usdt-x.volume_24h_usdt).slice(0,100);
      });
      setTrades(prev=>prev.map(t=>{
        if(!t.is_open)return t;
        const x=tickMap.get(t.pair.replace('/','')); if(!x)return t;
        const rate=Number(x.price||x.lastPrice||x.c||t.current_rate);
        const pnl=t.type==='short'?(t.open_rate-rate)*t.amount:(rate-t.open_rate)*t.amount;
        const margin=(t.open_rate*t.amount)/(t.leverage||1);
        const roe=margin>0?pnl/margin*100:0;
        return {...t,current_rate:rate,profit_abs:Number(pnl.toFixed(2)),profit_pct:Number(roe.toFixed(2)),profit_ratio:roe/100};
      }));
    };
    const load = async () => {
      try {
        const res = await fetch('/api/v1/live-tickers');
        const data = await res.json();
        if (active && Array.isArray(data.tickers)) {
  ...
  useEffect(() => {
    ...
    const fetchKlines = async () => {
      try {
        ...
        // Never generate synthetic market data. Keep the last verified Futures candles.
      }
    };
  ...
  const handleReloadStrategy = () => {
    addLog('INFO', 'Quant Brain yapılandırması yeniden yüklendi.');
  };
      ...
      <main className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-4 sm:py-6 flex-1 w-full overflow-x-hidden">
        {activeTab === 'dashboard' && (
          <div className="space-y-6">
            <AlgorithmBrain selectedPair={selectedPair} />
            <TradingDashboard
            metrics={metrics}
            trades={trades}
            markets={markets}
