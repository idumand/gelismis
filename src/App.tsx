...
export function App() {
  ...
  const [binanceEnvironment, setBinanceEnvironment] = useState<"live" | "demo">("demo");
    ...
    const fetchConfig = async () => {
      try {
        ...
        if (data && Object.keys(data).length > 0) {
          ...
          setBinanceEnvironment(data.exchange_environment === "demo" || data?.exchange?.environment === "demo" || data?.exchange?.environment === "testnet" ? "demo" : "live");
  ...
  // Hydrate full Futures market list and persisted whitelist/blacklist state.
  useEffect(() => {
    const loadPairlists = async () => {
      try {
        const [marketsRes, pairRes] = await Promise.all([fetch('/api/v1/markets'), fetch('/api/v1/pairlists')]);
        const marketsData = await marketsRes.json().catch(() => ({}));
        const pairData = await pairRes.json().catch(() => ({}));
        if (marketsRes.ok && Array.isArray(marketsData.markets)) {
          const white = new Set(Array.isArray(pairData.whitelist) ? pairData.whitelist : []);
          const black = new Set(Array.isArray(pairData.blacklist) ? pairData.blacklist : []);
          setMarkets(marketsData.markets.map((m: MarketPairInfo) => ({ ...m, in_whitelist: white.has(m.symbol), in_blacklist: black.has(m.symbol) })));
        }
      } catch {}
    };
    loadPairlists();
  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      try {
        ws = new WebSocket((binanceEnvironment === 'demo' ? 'wss://demo-fstream.binance.com/ws/!miniTicker@arr' : 'wss://fstream.binance.com/ws/!miniTicker@arr'));
  ...
  const persistPairlistChange = async (symbol: string, list: 'whitelist' | 'blacklist', enabled: boolean) => {
    const res = await fetch('/api/v1/pairlists', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ action: 'set', symbol, list, enabled })
    });
    const data = await res.json().catch(() => ({}));
    if (!res.ok || data?.ok === false) throw new Error(data?.error || 'Pairlist kaydedilemedi');
  };

  const handleToggleWhitelist = async (symbol: string) => {
    const current = markets.find(m => m.symbol === symbol);
    if (!current) return;
    const nextEnabled = !current.in_whitelist;
    const next = markets.map(m => m.symbol === symbol ? { ...m, in_whitelist: nextEnabled, in_blacklist: nextEnabled ? false : m.in_blacklist } : m);
    setMarkets(next);
    try { await persistPairlistChange(symbol, 'whitelist', nextEnabled); }
    catch { setMarkets(markets); addLog('ERROR', 'Whitelist değişikliği kaydedilemedi.'); }
  };

  const handleToggleBlacklist = async (symbol: string) => {
    const current = markets.find(m => m.symbol === symbol);
    if (!current) return;
    const nextEnabled = !current.in_blacklist;
    const next = markets.map(m => m.symbol === symbol ? { ...m, in_blacklist: nextEnabled, in_whitelist: nextEnabled ? false : m.in_whitelist } : m);
    setMarkets(next);
    try { await persistPairlistChange(symbol, 'blacklist', nextEnabled); }
    catch { setMarkets(markets); addLog('ERROR', 'Blacklist değişikliği kaydedilemedi.'); }
  };

  const handleAddPair = async (symbol: string) => {
    const formatted = symbol.trim().toUpperCase().includes('/') ? symbol.trim().toUpperCase() : `${symbol.trim().toUpperCase()}/USDT`;
    if (markets.some((m) => m.symbol === formatted)) return;
    const newMarket: MarketPairInfo = {
      symbol: formatted, base: formatted.split('/')[0] || 'CRYPTO', quote: 'USDT',
      price: 0, change_24h_pct: 0, volume_24h_usdt: 0, high_24h: 0, low_24h: 0,
      in_whitelist: true, in_blacklist: false, signal: 'NEUTRAL',
    };
    const next = [...markets, newMarket];
    setMarkets(next);
    try { await persistPairlistChange(formatted, 'whitelist', true); addLog('INFO', `Beyaz listeye yeni parite eklendi: ${formatted}`); }
    catch { setMarkets(markets); addLog('ERROR', 'Yeni parite kaydedilemedi.'); }
  };
  ...
  const handleToggleBotState = async (newState: BotState) => {
    try {
      const res = await fetch(newState === 'running' ? '/api/v1/start' : '/api/v1/stop', { method: 'POST' });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || data?.status === 'error') throw new Error(data?.message || 'Motor durumu değiştirilemedi.');
      setBotState(data?.state === 'running' ? 'running' : 'stopped');
    } catch (e) {
        ...
        {activeTab === 'config' && (
          <ConfigEditor
            ...
            onSaveConfig={async (jsonStr) => {
              ...
              try { const savedCfg = JSON.parse(jsonStr); setBinanceEnvironment((savedCfg?.exchange?.environment === 'demo' || savedCfg?.exchange?.environment === 'testnet') ? 'demo' : 'live'); } catch {}
              ...
              try {
                ...
                if (!configRes.ok || configData?.ok === false) throw new Error(configData?.error || 'Ayarlar kaydedilemedi.');
