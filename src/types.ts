...
export interface Trade {
  ...
  model_forecast_expected_value_usd?: number;
  model_forecast_hit_probability_pct?: number;
  model_forecast_risk_reward?: number;
  model_forecast_uncertainty_pct?: number;
  fee_open: number;
  fee_close?: number;
  target_hit?: boolean;
  max_favorable_move_pct?: number;
  max_adverse_move_pct?: number;
  adaptive_decision?: 'ENTER_NOW' | 'ENTER_BETTER' | 'WATCH' | 'IGNORE';
  adaptive_entry_score?: number;
  adaptive_target_pct?: number;
  adaptive_stop_loss_pct?: number;
  adaptive_min_probability_pct?: number;
  adaptive_breakeven_probability_pct?: number;
  adaptive_urgency?: number;
  adaptive_ai_probability?: number;
  adaptive_ai_decision?: string;
}
