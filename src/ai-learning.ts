...
export class AdaptiveLogisticAI {
  ...
  constructor(saved?:Partial<AIModelState>) {
    ...
    this.state = {
      ...
      weights: Array.isArray(saved?.weights) && saved!.weights.length===n ? saved!.weights.map(Number) : Array(n).fill(0),
      bias: finite(saved?.bias),
      mean: Array.isArray(saved?.mean) && saved!.mean.length===n ? saved!.mean.map(Number) : Array(n).fill(0.5),
      scale: Array.isArray(saved?.scale) && saved!.scale.length===n ? saved!.scale.map(v=>Math.max(0.01,Number(v))) : Array(n).fill(0.25),
      trainedSamples: Number(saved?.trainedSamples || 0),
      brier: saved?.brier ?? null,
      threshold: Number(saved?.threshold ?? 0.5),
