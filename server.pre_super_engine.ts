      ...
      for (const sym of Object.keys(activePositions)) {
        ...
        if ((closedPos as any).isRealBinance && (Date.now() - closedPos.openDate > 15000) && !activeSymbolsInExchange.has(sym)) {
          ...
          if (tradeIndex !== -1) {
            ...
            const pnl = closedPos.type === 'long' ? (closeRate-closedPos.entryPrice)*closedPos.amount : (closedPos.entryPrice-closeRate)*closedPos.amount;
            allTrades[tradeIndex].profit_abs = Number(pnl.toFixed(2));
            allTrades[tradeIndex].profit_pct = Number((((closedPos.entryPrice*closedPos.amount/Math.max(1,closedPos.leverage))>0 ? pnl/(closedPos.entryPrice*closedPos.amount/Math.max(1,closedPos.leverage))*100 : 0)).toFixed(2));
      ...
      try {
        ...
        // ================= ENTRY: ACTIVE QUANTITATIVE & ORDER FLOW SIGNAL ENGINE =================
          ...
          if (Object.keys(activePositions).length < maxOpenTrades) {
            ...
            const aiPass = aiProbability >= Math.min(68, breakeven + 8);
            const safetyHard = dataOk && notOverSpread && profitOk;
            const softScore = (Number(flow.edgeScore||0)*0.28) + (Number(flow.targetPathScore||0)*0.22) + (Math.abs(Number(flow.orderFlowGap||0))*0.18) + (Number(flow.dataQuality||0)*0.10) + (aiProbability*0.22);
            const entryOk = safetyHard && strongestScore >= 52 && directionalGap >= 7 && directionalFlow && pathOk && (aiPass || softScore >= 72);

            if (entryOk) {
              const type = strongest;
              const score = softScore + Math.max(0, Math.min(10, flow.movementPotentialPct * 2)) + Math.max(0, Math.min(5, flow.expectedNetProfitUSD));
              entryCandidates.push({
                ...
                aiProbability,
                aiDecision: aiPass ? 'ENTER_NOW' : 'ENTER_BETTER'
              });
            }
          }
        }
