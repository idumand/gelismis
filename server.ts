        ...
        // ================= ENTRY: ACTIVE QUANTITATIVE & ORDER FLOW SIGNAL ENGINE =================
          ...
          if (Object.keys(activePositions).length < maxOpenTrades) {
            ...
            const moneyOnlyGate = agentDirective.onlyMoneyFlow
              ? Number(agentDecision?.factors?.moneyFlow || 0) >= 0.72 && dataOk && notOverSpread
              : true;
            ...
            const agentStrategyGate = moneyOnlyGate && deepGate && agentFlowGate && agentTrendGate && (!agentDecision || agentDecision.action === 'ENTER_NOW' || (agentDirective.strategy !== 'deep_analysis' && agentDecision.action === 'WAIT' && aiHardPass));
...
function rankAgentMarkets() {
  ...
  const policy = { ...DEFAULT_COGNITIVE_POLICY, minConfidence: Number(aiGovernance.minConfidence||72), minProbability:Number(aiGovernance.minProbability||.68), maxUncertainty:Number(aiGovernance.maxUncertainty||.36), minRiskReward:Number(aiGovernance.minRiskReward||1.35), onlyMoneyFlow:Boolean(agentDirective.onlyMoneyFlow) };
...
async function executeAgentCommand(commandText:string) {
  ...
  if (cmd.intent === 'SCAN' || cmd.intent === 'OPEN_POSITION') {
    ...
    if (!agentDirective.allowEntries || botState !== 'running') return {ok:false,intent:cmd.intent,selected,message:'Giriş kararı hazır ancak ticaret motoru çalışmıyor; güvenlik nedeniyle emir gönderilmedi.'};
...
async function refreshAutonomousAI() {
  ...
  try {
    ...
    if (Object.keys(next).length) addEngineLog('INFO', `[AI OTONOM V4] ${Object.keys(next).length} aday deterministik agent tarafından onaylandı | ${describeDirective(agentDirective)}`);
    ...
    if (botState === 'running' && exchange && isExchangeAuthenticated && Object.keys(activePositions).length < Math.min(maxOpenTrades, agentDirective.maxPositions)) {
    ...
    addEngineLog('WARN', `[AI OTONOM V4] Tarama kullanılamadı: ${e?.message || e}`);
...
app.post("/api/v1/ai/autopilot", async (req, res) => {
  aiAutoPilotEnabled = Boolean(req.body?.enabled);
  if (autonomousAILoop) { clearInterval(autonomousAILoop); autonomousAILoop = null; }
  if (aiAutoPilotEnabled) {
    await refreshAutonomousAI();
  ...
  res.json({ ok: true, enabled: aiAutoPilotEnabled, candidates: Object.keys(autonomousAICandidates).length });
});
