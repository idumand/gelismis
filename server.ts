...
function startBinanceServerWebSocket() {
  ...
  try {
    ...
    try {
      const baseUrl = futuresWsBase();
      const urlFutures = `${baseUrl}/stream?streams=${streamNamesFutures}`;
      const env = getBinanceEnvironment();
      const envLabel = env === "demo" ? "DEMO (Testnet)" : "LIVE (Production)";
      addEngineLog("INFO", `[WebSocket] ${envLabel} ortamına bağlanılıyor: ${baseUrl}`);
      
      binanceWsClient = new WsClient(urlFutures);

      binanceWsClient.on('open', () => {
        addEngineLog("INFO", `✓ Binance Vadeli (Futures) ${envLabel} WebSocket aktif (${activeSymbols.length} parite).`);
...
app.post("/api/v1/exchange-keys", async (req, res) => {
  ...
  if (!apiKey || !secretKey || apiKey.trim() === "" || secretKey.trim() === "") {
    ...
    try {
      ...
      // IMPORTANT: Keep environment setting when clearing keys
      if (!conf.exchange.environment) conf.exchange.environment = "demo";
      fs.writeFileSync("config.json", JSON.stringify(conf, null, 2));
    } catch(e: any) {
  ...
  try {
    ...
    // Persist to config.json with environment explicitly set
    try {
      let conf: any = {};
      if (fs.existsSync("config.json")) {
        conf = JSON.parse(fs.readFileSync("config.json", "utf8"));
      ...
      // CRITICAL: Save the actual selected environment, not just demo/live boolean
      conf.exchange.environment = isDemo ? "demo" : "live";
      addEngineLog("INFO", `[API-KEYS] Ortam ayarı kaydedildi: ${conf.exchange.environment}`);
      fs.writeFileSync("config.json", JSON.stringify(conf, null, 2));
    } catch(e: any) {
      addEngineLog("ERROR", `[API-KEYS] Config kaydedilemedi: ${e?.message || e}`);
    ...
    const finalEnv = isDemo ? "demo" : "live";
    addEngineLog("INFO", `[API-KEYS] Başarılı: ${finalEnv} ortamına bağlandı, Bakiye: ${usdt} USDT`);
    return res.json({ success: true, balance_usdt: usdt, environment: finalEnv });
  } catch(e: any) {
