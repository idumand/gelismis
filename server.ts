...
import { AdaptiveLogisticAI, extractAIFeatures, aiFeatureNames, AITradeSample } from "./src/ai-learning.ts";
import { originalV7EntryGate } from "./src/original-engine-snapshot.ts";
import { askLLM, getLLMConfig, localFallback, type AIContext } from "./src/llm-orchestrator.ts";
import { parseAgentCommand, rankMarket, analyzePosition, applyCommandToDirective, describeDirective, DEFAULT_AGENT_DIRECTIVE, type AgentDirective, type AgentSide } from "./src/argos-ai-agent.ts";
import { DEFAULT_GOVERNANCE, HARD_SAFETY, governanceAdviceText, parseGovernanceCommand, sanitizeGovernance, type AIGovernanceState } from "./src/argos-ai-governance.ts";
import { ArgosCognitiveCoreV6, DEFAULT_COGNITIVE_POLICY, type CognitiveCoin, type CognitivePosition } from "./src/argos-ai-cognitive-v6.ts";
...
let maxOpenTrades = 1;
let aiAutoPilotEnabled = false;
let autonomousAILoop: NodeJS.Timeout | null = null;
let autonomousAIBusy = false;
let autonomousAICandidates: Record<string, { side: "long" | "short"; confidence: number; reason: string; updatedAt: number }> = {};
let aiDirective: Record<string, unknown> = {};
let agentDirective: AgentDirective = { ...DEFAULT_AGENT_DIRECTIVE };
let aiGovernance: AIGovernanceState = { ...DEFAULT_GOVERNANCE };
const aiGovernancePath = path.resolve(process.cwd(), "data", "argos_ai_governance.json");
let aiChatJournal: any[] = [];
let agentDecisionJournal: any[] = [];
let agentLastScanAt = 0;
const cognitiveCore = new ArgosCognitiveCoreV6();
let cognitiveUniverseSnapshot: any = null;
let cognitiveTopDecisions: any[] = [];
let cognitiveLastRunAt = 0;
const aiDirectivePath = path.resolve(process.cwd(), "data", "argos_ai_directive.json");
const agentDirectivePath = path.resolve(process.cwd(), "data", "argos_agent_directive.json");
const agentJournalPath = path.resolve(process.cwd(), "data", "argos_agent_journal.json");
const tradesPath = path.resolve(process.cwd(), "data", "argos_trades.json");
const aiChatJournalPath = path.resolve(process.cwd(), "data", "argos_ai_chat.json");
...
function loadAIDirective(){
  try {
    fs.mkdirSync(path.dirname(aiDirectivePath), { recursive: true });
    if (fs.existsSync(aiDirectivePath)) {
      const raw = JSON.parse(fs.readFileSync(aiDirectivePath, "utf8"));
      if (raw && typeof raw === "object") aiDirective = raw;
    }
    if (fs.existsSync(agentDirectivePath)) {
      const raw = JSON.parse(fs.readFileSync(agentDirectivePath, "utf8"));
      if (raw && typeof raw === "object") agentDirective = { ...DEFAULT_AGENT_DIRECTIVE, ...raw };
    }
    if (fs.existsSync(agentJournalPath)) {
      const raw = JSON.parse(fs.readFileSync(agentJournalPath, "utf8"));
      if (Array.isArray(raw)) agentDecisionJournal = raw.slice(-5000);
    }
    if (fs.existsSync(aiGovernancePath)) {
      const raw = JSON.parse(fs.readFileSync(aiGovernancePath, "utf8"));
      if (raw && typeof raw === 'object') aiGovernance = sanitizeGovernance(raw, DEFAULT_GOVERNANCE, 'system', 'Persisted AI governance').state;
    }
    if (fs.existsSync(aiChatJournalPath)) {
      const raw = JSON.parse(fs.readFileSync(aiChatJournalPath, "utf8"));
      if (Array.isArray(raw)) aiChatJournal = raw.slice(-3000);
    }
  } catch {}
}
function persistAIGovernance(){
  try { fs.mkdirSync(path.dirname(aiGovernancePath), { recursive: true }); fs.writeFileSync(aiGovernancePath, JSON.stringify(aiGovernance, null, 2)); } catch {}
}
function persistAIChat(entry:any){
  aiChatJournal.push(entry); if (aiChatJournal.length > 3000) aiChatJournal = aiChatJournal.slice(-3000);
  try { fs.mkdirSync(path.dirname(aiChatJournalPath), { recursive: true }); fs.writeFileSync(aiChatJournalPath, JSON.stringify(aiChatJournal)); } catch {}
}
function persistAIDirective(){
  try { fs.mkdirSync(path.dirname(aiDirectivePath), { recursive: true }); fs.writeFileSync(aiDirectivePath, JSON.stringify(aiDirective, null, 2)); } catch {}
  try { fs.mkdirSync(path.dirname(agentDirectivePath), { recursive: true }); fs.writeFileSync(agentDirectivePath, JSON.stringify(agentDirective, null, 2)); } catch {}
}
function persistAgentDecision(entry:any){
  agentDecisionJournal.push(entry);
  if (agentDecisionJournal.length > 5000) agentDecisionJournal = agentDecisionJournal.slice(-5000);
  try { fs.mkdirSync(path.dirname(agentJournalPath), { recursive: true }); fs.writeFileSync(agentJournalPath, JSON.stringify(agentDecisionJournal)); } catch {}
}
  ...
  try {
    ...
    if (fs.existsSync(aiJournalPath)) {
      ...
      if (Array.isArray(raw)) {
        const required = new Set(aiFeatureNames());
        aiJournal = raw.filter((x:any) => {
          if (!x || !Number.isFinite(Number(x.label))) return false;
          for (const key of required) if (!Number.isFinite(Number(x[key]))) return false;
          return true;
        });
      }
    }
...
function recordAITradeOutcome(trade:any){
  ...
  if (aiJournal.length >= 30) {
    adaptiveAI.fit(aiJournal);
    try {
      const modelPath = path.resolve(process.cwd(), "data", "argos_ai_model.json");
      fs.mkdirSync(path.dirname(modelPath), { recursive: true });
      fs.writeFileSync(modelPath, JSON.stringify(adaptiveAI.state, null, 2));
    } catch {}
  }
}
...
async function initializeExchange() {
  try {
    ...
    targetLeverage = Math.min(aiGovernance.maxLeverage, Math.round(toFiniteNumber(conf?.leverage, SETTINGS_DEFAULTS.leverage, 1, 125)));
...
function analyzeMicrostructure(
  ...
  // Directional price trend from the observed series only. This is used as a
  // secondary regime feature; it never overrides order-flow/liquidity evidence.
  const trendScore = prices.length >= 12
    ? clamp(((prices[prices.length-1] / prices[Math.max(0, prices.length-12)] - 1) * 100) * 7.5 + ((currentPrice > vwap ? 1 : -1) * 18), -100, 100)
    : 0;
  ...
  return {
    ...
    longRiskReward:longScenario.riskReward,
    shortRiskReward:shortScenario.riskReward,
    trendScore,
    longTrendScore:trendScore,
    shortTrendScore:-trendScore,
...
function startBinanceServerWebSocket() {
  ...
  try {
    ...
    const deepSymbols = [...new Set([...activeSymbols, ...Object.keys(activePositions)])];
    const selectiveStreams = deepSymbols
      .map(c => `${c.replace('/', '').toLowerCase()}@depth@100ms/${c.replace('/', '').toLowerCase()}@aggTrade`)
      .join('/');
    // All-market streams keep every listed futures symbol visible to the AI at low bandwidth;
    // deep order-flow streams remain focused on the active scan universe and open positions.
    const streamNamesFutures = ['!ticker@arr','!bookTicker','!forceOrder@arr',selectiveStreams].filter(Boolean).join('/');
    
    // 1. Binance Futures WebSocket
    try {
      const wsPath = getBinanceEnvironment() === 'demo' ? '/stream' : '/public/stream';
      const urlFutures = `${futuresWsBase()}${wsPath}?streams=${streamNamesFutures}`;
...
function handleWsMessage(raw: any) {
  try {
    ...
    // All-market ticker stream arrives as an array; process it recursively so the AI has a
    // continuously refreshed snapshot of the full USDT perpetual universe.
    if (Array.isArray(data)) {
      latestTickersCache = data;
      for (const item of data) {
        const us = String(item?.s||'').toUpperCase();
        if (!us.endsWith('USDT')) continue;
        const fsym = `${us.slice(0,-4)}/USDT`;
        const price = Number(item?.c||0), pct = Number(item?.P||0), vol = Number(item?.q||0);
        if (!(price>0)) continue;
        latestMetricsPerCoin[fsym] = { ...(latestMetricsPerCoin[fsym]||{}), currentPrice:price, price, change_24h_pct:pct, volume_24h_usdt:vol, quoteVolume:vol, updatedAt:Number(item?.E||Date.now()), allMarketTicker:true };
      }
      return;
    }

    if (stream.includes('!bookTicker') || data.e === 'bookTicker') {
      const fsym = formattedSym;
      if (!latestMetricsPerCoin[fsym]) latestMetricsPerCoin[fsym] = {};
      const bid=Number(data.b||0), ask=Number(data.a||0), bidQty=Number(data.B||0), askQty=Number(data.A||0);
      latestMetricsPerCoin[fsym].bestBid=bid; latestMetricsPerCoin[fsym].bestAsk=ask; latestMetricsPerCoin[fsym].bestBidQty=bidQty; latestMetricsPerCoin[fsym].bestAskQty=askQty;
      latestMetricsPerCoin[fsym].microSpreadPct=bid>0&&ask>0?((ask-bid)/((ask+bid)/2))*100:0;
      latestMetricsPerCoin[fsym].bookTickerUpdatedAt=Number(data.E||Date.now());
      latestMetricsPerCoin[fsym].updatedAt=Math.max(Number(latestMetricsPerCoin[fsym].updatedAt||0),Number(data.E||Date.now()));
      if (!(latestMetricsPerCoin[fsym].currentPrice>0) && bid>0&&ask>0) latestMetricsPerCoin[fsym].currentPrice=(bid+ask)/2;
      return;
    }

    if (stream.includes('!forceOrder') || data.e === 'forceOrder') {
      const fsym = formattedSym;
      const o=data.o||data; const qty=Number(o.q||o.l||0), price=Number(o.ap||o.p||0), side=String(o.S||o.side||'').toUpperCase();
      if (!latestMetricsPerCoin[fsym]) latestMetricsPerCoin[fsym] = {};
      const notional=qty*price;
      latestMetricsPerCoin[fsym].lastLiquidationUSD = notional;
      latestMetricsPerCoin[fsym].lastLiquidationSide = side;
      latestMetricsPerCoin[fsym].liquidationUpdatedAt = Number(data.E||Date.now());
      latestMetricsPerCoin[fsym].liquidationPressure = clamp((Number(latestMetricsPerCoin[fsym].liquidationPressure||0)*0.85)+(side==='BUY'?1:-1)*Math.min(100,notional/100000),-100,100);
      return;
    }
...
async function updateMarketDataAndExecute() {
  ...
  await runWithConcurrency(scanSymbols, 16, async (symbol) => {
      try {
        ...
        // AI v2 is evaluated after all real market features are assembled. The
        // resulting probability is informational until the hard execution gates pass.
        const liveLongAI = adaptiveAI.predictDetailed(extractAIFeatures({ ...flow, rsi: currentRSI, ema9: currentEMA9, ema21: currentEMA21 }, 50, 'long'));
        const liveShortAI = adaptiveAI.predictDetailed(extractAIFeatures({ ...flow, rsi: currentRSI, ema9: currentEMA9, ema21: currentEMA21 }, 50, 'short'));
        const liveAIIsLong = liveLongAI.score >= liveShortAI.score;
        const liveAI = liveAIIsLong ? liveLongAI : liveShortAI;
        const absTrend = Math.abs(Number(flow.trendScore || 0));
        const volPct = Number(flow.stdDev || 0) * 100;
        const marketRegime = flow.dataQuality < 70 ? 'DATA_WEAK'
          : volPct >= 4.5 && absTrend < 20 ? 'HIGH_VOL_CHOP'
          : absTrend >= 45 ? 'TRENDING'
          : volPct >= 3 ? 'VOLATILE'
          : 'RANGE';
        ...
        latestMetricsPerCoin[symbol] = {
          ...
          atr: currentATR,
          ...flow,
          aiProbability: Number((liveAI.probability * 100).toFixed(2)),
          aiLearnedProbability: Number((liveAI.learnedProbability * 100).toFixed(2)),
          aiRulePrior: Number((liveAI.rulePrior * 100).toFixed(2)),
          aiConfidence: Number(liveAI.confidence.toFixed(2)),
          aiUncertainty: Number((liveAI.uncertainty * 100).toFixed(2)),
          aiScore: Number(liveAI.score.toFixed(2)),
          aiAction: liveAI.action,
          aiReasons: liveAI.reasons,
          aiLongProbability: Number((liveLongAI.probability * 100).toFixed(2)),
          aiShortProbability: Number((liveShortAI.probability * 100).toFixed(2)),
          aiDirection: liveAIIsLong ? 'long' : 'short',
          aiModelVersion: adaptiveAI.state.version,
          marketRegime,
          agentStrategy: agentDirective.strategy,
          agentDirective: { ...agentDirective },
          agentDataFreshnessSec: Number(((Date.now() - Number(latestMetricsPerCoin[symbol]?.updatedAt || Date.now())) / 1000).toFixed(2))
        };

        // ARGOS Agent V3 evaluates both directions continuously, even while the trade engine is stopped.
        try {
          const agentLive = rankMarket([{ symbol, ...flow, currentPrice, price: currentPrice, volume_24h_usdt: Number(ticker?.quoteVolume || 0), updatedAt: Date.now() } as any], agentDirective, (m:any, side:AgentSide) => adaptiveAI.predict(extractAIFeatures(m, Number(m.adaptiveUrgency ?? 50), side)));
          const agLong:any = agentLive.find((d:any)=>d.side==='long');
          const agShort:any = agentLive.find((d:any)=>d.side==='short');
          const agBest:any = agentLive[0];
          Object.assign(latestMetricsPerCoin[symbol], {
            agentScore: Number(agBest?.score || 0),
            agentProbability: Number((agBest?.probability || 0) * 100),
            agentConfidence: Number(agBest?.confidence || 0),
            agentUncertainty: Number((agBest?.uncertainty || 0) * 100),
            agentAction: agBest?.action || 'IGNORE',
            agentDirection: agBest?.side || 'long',
            agentLongScore: Number(agLong?.score || 0),
            agentShortScore: Number(agShort?.score || 0),
            agentReasons: agBest?.reasons || [],
            agentWarnings: agBest?.warnings || []
          });
          agentLastScanAt = Date.now();
        } catch {}
        ...
        if (pos) {
          ...
          const liveAI = adaptiveAI.predictDetailed(extractAIFeatures(flow, Number(flow.adaptiveUrgency ?? 50), pos.type));
          const liveAIProbabilityPct = liveAI.probability * 100;
          const liveAIWeak = liveAIProbabilityPct < 45 || liveAI.action === 'IGNORE';
          const liveAISeverelyWeak = liveAIProbabilityPct < 38 || (liveAI.uncertainty > 0.62 && liveAI.confidence < 50);
          ...
          if (positivePnl) {
            const deterioration = [bookWeak, moneyWeak, largeMoneyWeak, consumptionWeak, significantPeakGiveback, gapTrend < -8, liveAIWeak].filter(Boolean).length;
            if (deterioration >= 3 || (pnlDrawdown >= 0.35 && deterioration >= 2) || (liveAISeverelyWeak && deterioration >= 2)) {
              shouldExit = true;
          ...
          if (!shouldExit && !positivePnl && pos.pnlHistory.length >= 4 && liveAISeverelyWeak && (bookWeak || moneyWeak || largeMoneyWeak)) {
            shouldExit = true;
            exitReason = `AI Zarar Koruma: canlı kazanma olasılığı %${liveAIProbabilityPct.toFixed(1)} seviyesine düştü | Güven %${liveAI.confidence.toFixed(0)} | Belirsizlik %${(liveAI.uncertainty*100).toFixed(0)}`;
          }
          ...
          // ARGOS AI Agent V4: continuously re-evaluates the live thesis.
          // This is a second, explainable safety layer; it never disables the exchange STOP_MARKET.
          if (activePositions[symbol] && agentDirective.autoManagePositions) {
            const livePos:any = activePositions[symbol];
            const agentPos = analyzePosition({
              symbol, side: livePos.type, entryPrice: livePos.entryPrice, currentPrice,
              pnlUSD, roePct, openedAt: livePos.openDate, peakNetPnl: livePos.peakNetPnl || 0
            } as any, { ...flow, ...latestMetricsPerCoin[symbol], symbol }, agentDirective);
            agentLastScanAt = Date.now();
            persistAgentDecision({ type:'POSITION_OBSERVATION', symbol, side:livePos.type, action:agentPos.action, score:agentPos.score, probability:agentPos.probability, confidence:agentPos.confidence, uncertainty:agentPos.uncertainty, pnlUSD, roePct, generatedAt:Date.now() });
            if (agentPos.action === 'EXIT_NOW' && !pendingExits.has(symbol)) {
              await executeExit(symbol, `AI Agent V4: tez bozuldu | ${agentPos.reasons.slice(-3).join(' | ')}`, currentPrice);
            }
            const cognitivePos = cognitiveCore.positionReview(
              { symbol, side: livePos.type, entryPrice: livePos.entryPrice, currentPrice, pnlUSD, roePct, openedAt: livePos.openDate, peakPnlUSD: livePos.peakNetPnl || 0 } as CognitivePosition,
              ({ symbol, ...latestMetricsPerCoin[symbol], price: currentPrice, updatedAt: Date.now() } as unknown) as CognitiveCoin,
              getAgentMarketUniverse() as CognitiveCoin[],
              { ...DEFAULT_COGNITIVE_POLICY, minConfidence:Number(aiGovernance.minConfidence||72), minProbability:Number(aiGovernance.minProbability||.68), maxUncertainty:Number(aiGovernance.maxUncertainty||.36), minRiskReward:Number(aiGovernance.minRiskReward||1.35) },
              Date.now()
            );
            persistAgentDecision({ type:'COGNITIVE_POSITION_REVIEW', symbol, side:livePos.type, action:cognitivePos.action, score:cognitivePos.score, probability:cognitivePos.probability, confidence:cognitivePos.confidence, uncertainty:cognitivePos.uncertainty, pnlUSD, generatedAt:Date.now() });
            if (cognitivePos.action === 'EXIT_NOW' && !pendingExits.has(symbol) && activePositions[symbol]) {
              await executeExit(symbol, `CORTEX V6: pozisyon tezi bozuldu | ${cognitivePos.warnings.slice(-2).join(' | ') || cognitivePos.reasons.slice(-2).join(' | ')}`, currentPrice);
            }
          }
        ...
        // ================= ENTRY: ACTIVE QUANTITATIVE & ORDER FLOW SIGNAL ENGINE =================
          ...
          if (Object.keys(activePositions).length < maxOpenTrades) {
            ...
            const aiFeatures = extractAIFeatures(flow, Number(flow.adaptiveUrgency ?? 50), strongest);
            const aiDecision = adaptiveAI.predictDetailed(aiFeatures);
            const aiProbability = aiDecision.probability * 100;
            const agentRanked = rankMarket([{ symbol, ...flow, currentPrice, updatedAt: Date.now(), volume_24h_usdt: Number(ticker?.quoteVolume || latestMetricsPerCoin[symbol]?.volume_24h_usdt || 0) } as any], agentDirective, (m:any, side:AgentSide) => adaptiveAI.predict(extractAIFeatures(m, Number(m.adaptiveUrgency ?? 50), side)));
            const agentDecision = agentRanked.find((d:any) => d.side === strongest) || agentRanked[0];
            const aiUncertaintyPct = aiDecision.uncertainty * 100;
            const aiConfidence = aiDecision.confidence;
            const learnedThreshold = Math.max(52, Math.min(82, Number(adaptiveAI.state.threshold || 0.5) * 100));
            const capitalBattleOk = strongestScore >= 58 && directionalGap >= 10 && selectedResistance <= 58 && selectedProbability >= 55;
            const regimeOk = Number(flow.dataQuality || 0) >= 78 && Number(flow.targetPathScore || 0) >= 62 && Number(flow.volumeRatio || 1) >= 0.70;
            const economicsOk = selectedEV > Math.max(0.08, minExpectedNetProfitUSD * 0.25) && Number(flow.selectedRiskReward || 0) >= 0.45;
            const safetyHard = dataOk && notOverSpread && profitOk && evOk && regimeOk && economicsOk;
            const aiHardPass = aiDecision.action === 'ENTER_NOW' && aiProbability >= learnedThreshold && aiConfidence >= 66 && aiUncertaintyPct <= 45;
            const aiBetterPass = aiDecision.action === 'ENTER_BETTER' && aiProbability >= Math.max(58, learnedThreshold - 5) && aiConfidence >= 60 && aiUncertaintyPct <= 50;
            const softScore = (Number(flow.edgeScore||0)*0.16) + (Number(flow.targetPathScore||0)*0.14) + (Math.abs(Number(flow.orderFlowGap||0))*0.10) + (Number(flow.dataQuality||0)*0.08) + (aiProbability*0.22) + (aiConfidence*0.12) + ((100-selectedResistance)*0.10) + (Number(flow.selectedRiskReward||0)*4*0.08);
            // LOCKED ORIGINAL V7 decision: this expression remains the untouched baseline.
            const originalEntryOk = originalV7EntryGate({ flow, minExpectedMovePct: activeMinExpectedMovePct, minExpectedNetProfitUSD });
            const llmCandidate = aiAutoPilotEnabled ? autonomousAICandidates[symbol] : undefined;
            const directiveSide = String(aiDirective.side || "both");
            const directiveSideOk = directiveSide === "both" || directiveSide === strongest;
            const llmSideOk = !llmCandidate || llmCandidate.side === strongest;
            const llmBoost = llmCandidate ? Math.max(0, Math.min(8, (llmCandidate.confidence - 60) * 0.20)) : 0;
            const moneyFlowNet = Number(flow.netInflowUSDT ?? flow.netInflowUSD ?? flow.net_inflow_usdt ?? flow.netCapitalFlow ?? 0);
            const moneyOnlyGate = agentDirective.onlyMoneyFlow
              ? Boolean(agentDecision) && (Number(agentDecision?.factors?.moneyFlow || 0) >= 0.56 || Math.abs(moneyFlowNet) > 0) && dataOk && notOverSpread
              : true;
            const deepGate = agentDirective.requireDeepAnalysis
              ? Boolean(agentDecision && agentDecision.confidence >= agentDirective.minConfidence && agentDecision.uncertainty <= 0.42 && agentDecision.probability >= agentDirective.minProbability)
              : true;
            const agentFlowGate = !agentDirective.requireOrderFlowAlignment || Number(agentDecision?.factors?.flow || 0) >= 0.55;
            const agentTrendGate = !agentDirective.requireTrendAlignment || Number(flow.trendScore || 0) * (strongest === 'long' ? 1 : -1) >= 8;
            const agentStrategyGate = agentDirective.onlyMoneyFlow
              ? moneyOnlyGate
              : moneyOnlyGate && deepGate && agentFlowGate && agentTrendGate && (!agentDecision || agentDecision.action === 'ENTER_NOW' || (agentDirective.strategy !== 'deep_analysis' && agentDecision.action === 'WAIT' && aiHardPass));
            const adaptiveEntryOk = (agentDirective.onlyMoneyFlow ? agentStrategyGate : safetyHard && capitalBattleOk && directionalFlow && pathOk && movementOk && directiveSideOk && llmSideOk && (aiHardPass || (aiBetterPass && softScore + llmBoost >= 70)) && agentStrategyGate);
            ...
            if (entryOk) {
              ...
              const score = engineMode === "original"
                ...
                : softScore + llmBoost + Math.max(0, Math.min(10, flow.movementPotentialPct * 2)) + Math.max(0, Math.min(5, flow.expectedNetProfitUSD));
              entryCandidates.push({
                ...
                aiDecision: engineMode === "original" ? 'ORIGINAL' : aiDecision.action,
                agentScore: Number(agentDecision?.score || 0),
                agentProbability: Number(agentDecision?.probability || 0),
                agentConfidence: Number(agentDecision?.confidence || 0),
                agentUncertainty: Number(agentDecision?.uncertainty || 0),
                agentStrategy: agentDirective.strategy
              });
            }
          }
...
async function executeEntry(symbol: string, type: "long" | "short", currentPrice: number): Promise<boolean> {
  ...
  activePositions[symbol] = {
    ...
    aiModelVersion: adaptiveAI.state.version,
    aiEntryUncertainty: adaptiveAI.predictDetailed(extractAIFeatures(latestMetricsPerCoin[symbol], Number(latestMetricsPerCoin[symbol]?.adaptiveUrgency ?? 50), type)).uncertainty,
    aiEntryConfidence: adaptiveAI.predictDetailed(extractAIFeatures(latestMetricsPerCoin[symbol], Number(latestMetricsPerCoin[symbol]?.adaptiveUrgency ?? 50), type)).confidence,
    lastMarketUpdateAt: Date.now()
  };
...
async function fetchPublicAINews() {
  if (Date.now() - aiNewsFetchedAt < 5 * 60 * 1000) return aiNewsCache;
  const feeds = [
  ...
  for (const url of feeds) {
    try {
      const r = await fetch(url, { headers: { "user-agent": "ARGOS-AI/1.0" } });
      if (!r.ok) continue;
      const xml = await r.text();
      const re = /<item[\s\S]*?<title>(?:<!\[CDATA\[)?([\s\S]*?)(?:\]\]>)?<\/title>[\s\S]*?<link>([\s\S]*?)<\/link>[\s\S]*?(?:<pubDate>([\s\S]*?)<\/pubDate>)?[\s\S]*?<\/item>/gi;
      let m: RegExpExecArray | null;
      let count = 0;
      while ((m = re.exec(xml)) && count < 12) {
        items.push({ title: String(m[1] || '').replace(/<[^>]+>/g, '').trim(), link: String(m[2] || '').trim(), publishedAt: String(m[3] || '').trim(), source: url.includes('cointelegraph') ? 'Cointelegraph' : 'CoinDesk' });
        count++;
      }
    } catch {}
  }
  aiNewsCache = items.slice(0, 24);
  aiNewsFetchedAt = Date.now();
  return aiNewsCache;
}

function buildAIContext(news: any[] = []): AIContext {
  const coins = allFuturesMarketsList.map((symbol) => {
    const m: any = latestMetricsPerCoin[symbol] || {};
    return {
      symbol,
      price: Number(m.currentPrice || 0),
      change_24h_pct: Number(m.change_24h_pct || 0),
      volume_24h_usdt: Number(m.volume_24h_usdt || 0),
      netInflowUSDT: Number(m.netInflowUSDT ?? m.net_inflow_usdt ?? m.netCapitalFlow ?? 0),
      longPressure: Number(m.longPressure ?? m.longAdvantage ?? 0),
      shortPressure: Number(m.shortPressure ?? m.shortAdvantage ?? 0),
      largeLongUSDT: Number(m.largeLongUSDT ?? m.largeBuyUSDT ?? 0),
      largeShortUSDT: Number(m.largeShortUSDT ?? m.largeSellUSDT ?? 0),
      obi: Number(m.obi ?? m.orderBookImbalance ?? 0),
      deepScore: Number(m.deepScore ?? 0),
      expectedMovePct: Number(m.expectedMovePct ?? 0),
      targetConfidence: Number(m.targetConfidence ?? 0),
      aiScore: Number(m.aiScore ?? m.deepScore ?? 0),
      aiProbability: Number(m.aiProbability ?? 0),
      aiConfidence: Number(m.aiConfidence ?? 0),
      aiUncertainty: Number(m.aiUncertainty ?? 0),
      aiAction: String(m.aiAction ?? 'WATCH'),
      aiLongProbability: Number(m.aiLongProbability ?? 0),
      aiShortProbability: Number(m.aiShortProbability ?? 0),
      agentScore: Number(m.agentScore ?? 0),
      agentProbability: Number(m.agentProbability ?? 0),
      agentConfidence: Number(m.agentConfidence ?? 0),
      agentUncertainty: Number(m.agentUncertainty ?? 0),
      agentAction: String(m.agentAction ?? 'IGNORE'),
      agentDirection: String(m.agentDirection ?? 'long'),
      agentReasons: Array.isArray(m.agentReasons) ? m.agentReasons.slice(0,6) : [],
      agentWarnings: Array.isArray(m.agentWarnings) ? m.agentWarnings.slice(0,6) : [],
      marketRegime: String(m.marketRegime ?? 'UNKNOWN'),
      trendScore: Number(m.trendScore ?? 0),
      orderFlowGap: Number(m.orderFlowGap ?? 0),
      inflowMomentum: Number(m.inflowMomentum ?? 0),
      largeTradeScore: Number(m.largeTradeScore ?? 0),
      liquidityConsumptionScore: Number(m.liquidityConsumptionScore ?? 0),
      targetPathScore: Number(m.targetPathScore ?? 0),
      selectedExpectedValueUSD: Number(m.selectedExpectedValueUSD ?? m.expectedNetProfitUSD ?? 0),
      selectedRiskReward: Number(m.selectedRiskReward ?? m.riskReward ?? 0),
      dataQuality: Number(m.dataQuality ?? 0),
      updatedAt: Number(m.updatedAt ?? 0),
      signal: m.signal || m.adaptiveDecision || 'NEUTRAL'
    };
  });
  return {
    ...
    botState,
    directive: { ...aiDirective, governance: aiGovernance },
    account: { authenticated: isExchangeAuthenticated, balanceUSDT: null },
    positions: Object.values(activePositions).map((p: any) => ({ symbol: p.pair, side: p.type, entryPrice: p.entryPrice, currentPrice: p.currentPrice, pnl: p.unrealizedPnl, stop: p.baseStopPrice, target: p.modelTargetPrice })),
    coins,
    news,
    engine: { mode: engineMode, tradingSource, maxOpenTrades, stakeAmount: activeStakeAmount, leverage: targetLeverage, minExpectedMovePct: activeMinExpectedMovePct, governance: aiGovernance, hardSafety: HARD_SAFETY, cortex: { version: cognitiveCore.version, state: cognitiveUniverseSnapshot, top: cognitiveTopDecisions.slice(0,40), lastRunAt: cognitiveLastRunAt, universeCount: allFuturesMarketsList.length, liveTickerCount: latestTickersCache.length } }
  };
}
function applyUserDirective(text: string) {
  const parsed = parseAgentCommand(text);
  agentDirective = applyCommandToDirective(agentDirective, parsed);
  aiDirective = { ...aiDirective, ...parsed.extracted, strategy: agentDirective.strategy, onlyMoneyFlow: agentDirective.onlyMoneyFlow, requireDeepAnalysis: agentDirective.requireDeepAnalysis, requireOrderFlowAlignment: agentDirective.requireOrderFlowAlignment, requireTrendAlignment: agentDirective.requireTrendAlignment, minConfidence: agentDirective.minConfidence, minProbability: agentDirective.minProbability, maxOpenTrades: agentDirective.maxPositions };
  if (Number.isFinite(Number(parsed.maxPositions))) maxOpenTrades = Math.max(1, Math.min(20, Number(parsed.maxPositions)));
  persistAIDirective();
  addEngineLog('SYSTEM', `[AI AGENT] Direktif güncellendi: ${describeDirective(agentDirective)} | ${text}`);
  return { parsed, directive: agentDirective };
}

function applyAIGovernanceFromText(text:string, actor:'user'|'ai'='ai') {
  const parsed = parseGovernanceCommand(text);
  if (!parsed.recognized) return { recognized:false, changed:[], blocked:[], state:aiGovernance, parsed };
  const result = sanitizeGovernance(parsed.changes, aiGovernance, actor, text);
  aiGovernance = result.state;
  // Mirror safe, user-visible soft controls into the existing agent directive.
  agentDirective = { ...agentDirective, allowEntries: aiGovernance.allowEntries, autoManagePositions: aiGovernance.autoManagePositions, minConfidence: aiGovernance.minConfidence, minProbability: aiGovernance.minProbability, maxUncertainty: aiGovernance.maxUncertainty, minRiskReward: aiGovernance.minRiskReward, maxPositions: aiGovernance.maxPositions, riskPerTradePct: aiGovernance.riskPerTradePct, allowScaleIn: aiGovernance.allowScaleIn, updatedAt: Date.now() };
  activeStopLossPct = aiGovernance.stopLossPct;
  activeStakeAmount = aiGovernance.stakeAmountUSDT;
  maxOpenTrades = aiGovernance.maxPositions;
  targetLeverage = Math.min(targetLeverage, aiGovernance.maxLeverage);
  persistAIGovernance(); persistAIDirective();
  addEngineLog('SYSTEM', `[AI GOVERNANCE] ${actor.toUpperCase()} politika güncelledi | ${governanceAdviceText(aiGovernance)}${result.blocked.length ? ` | BLOKE: ${result.blocked.join(', ')}` : ''}`);
  return { ...result, parsed, state:aiGovernance };
}

function aiRecommendationSummary() {
  const rankings = rankAgentMarkets();
  const top = rankings.filter((x:any)=>x.action!=='IGNORE').slice(0,6);
  const best = top[0];
  if (!best) return {summary:'Şu anda güvenilir giriş adayı yok.', recommendations:[], marketBreadth:{coins:getAgentMarketUniverse().length}};
  return {
    summary:`En güçlü mevcut aday ${best.symbol} ${best.side.toUpperCase()}; karar ${best.action}, olasılık %${(best.probability*100).toFixed(1)}, güven %${best.confidence.toFixed(1)}, belirsizlik %${(best.uncertainty*100).toFixed(1)}.`,
    recommendations:top.map((d:any)=>({symbol:d.symbol,side:d.side,action:d.action,score:Number(d.score.toFixed(1)),probability:Number((d.probability*100).toFixed(1)),confidence:Number(d.confidence.toFixed(1)),uncertainty:Number((d.uncertainty*100).toFixed(1)),riskReward:Number(d.riskReward.toFixed(2)),expectedValue:Number(d.expectedValue.toFixed(3)),reasons:d.reasons.slice(0,5),warnings:d.warnings.slice(0,4)})),
    marketBreadth:{coins:getAgentMarketUniverse().length,enterNow:rankings.filter((x:any)=>x.action==='ENTER_NOW').length}
  };
}

function getAgentMarketUniverse() {
  const symbols = allFuturesMarketsList.length ? allFuturesMarketsList : whitelistCoins;
  const tickerMap = new Map<string, any>();
  for (const t of latestTickersCache) {
    const clean = String(t?.symbol || t?.s || '').toUpperCase();
    if (clean) tickerMap.set(clean, t);
  }
  return symbols.map((symbol:any) => {
    const m:any = latestMetricsPerCoin[symbol] || {};
    const clean = String(symbol).replace('/', '').toUpperCase();
    const tick = tickerMap.get(clean);
    const tickPrice = Number(tick?.lastPrice || tick?.price || 0);
    const tickVolume = Number(tick?.quoteVolume || 0);
    return {
      symbol,
      ...m,
      currentPrice: Number(m.currentPrice || tickPrice || 0),
      price: Number(m.currentPrice || tickPrice || 0),
      volume_24h_usdt: Number(m.volume_24h_usdt || tickVolume || 0),
      change_24h_pct: Number(m.change_24h_pct ?? tick?.priceChangePercent ?? 0),
      updatedAt: Number(m.updatedAt || (latestTickersCache.length ? Date.now() - 1000 : 0)),
    };
  }).filter((x:any) => x.price > 0);
}

function rankAgentMarkets() {
  const coins = getAgentMarketUniverse();
  const positions: CognitivePosition[] = agentPositionSummary().map((p:any)=>({symbol:p.symbol,side:p.side,entryPrice:p.entryPrice,currentPrice:p.currentPrice,pnlUSD:p.pnlUSD,roePct:p.roePct,openedAt:p.openedAt,peakPnlUSD:p.peakNetPnl}));
  const moneyOnly = Boolean(agentDirective.onlyMoneyFlow);
  const policy = {
    ...DEFAULT_COGNITIVE_POLICY,
    minScore: moneyOnly ? 55 : Number(DEFAULT_COGNITIVE_POLICY.minScore),
    minConfidence: moneyOnly ? 56 : Number(aiGovernance.minConfidence || DEFAULT_COGNITIVE_POLICY.minConfidence),
    minProbability: moneyOnly ? 0.54 : Number(aiGovernance.minProbability || DEFAULT_COGNITIVE_POLICY.minProbability),
    maxUncertainty: moneyOnly ? 0.52 : Number(aiGovernance.maxUncertainty || DEFAULT_COGNITIVE_POLICY.maxUncertainty),
    minRiskReward: moneyOnly ? 0.90 : Number(aiGovernance.minRiskReward || DEFAULT_COGNITIVE_POLICY.minRiskReward),
    antiChop: moneyOnly ? false : DEFAULT_COGNITIVE_POLICY.antiChop,
    onlyMoneyFlow: moneyOnly
  };
  const cognitive = cognitiveCore.analyzeUniverse(coins as CognitiveCoin[], positions, policy, 240);
  cognitiveTopDecisions=cognitive; cognitiveLastRunAt=Date.now(); cognitiveUniverseSnapshot=cognitiveCore.state();
  const legacy=rankMarket(coins as any, agentDirective, (m:any, side:AgentSide) => { try { return adaptiveAI.predict(extractAIFeatures(m, Number(m.adaptiveUrgency ?? 50), side)); } catch { return 0.5; } });
  const map=new Map(legacy.map((x:any)=>[x.symbol,x]));
  return cognitive.map((d:any)=>{ const l:any=map.get(d.symbol); return { ...d, legacy:l, action:d.action, score:d.score, probability:d.probability, confidence:d.confidence, uncertainty:d.uncertainty, riskReward:d.riskReward, expectedValue:d.expectedValue, marketRegime:d.regime, factors:d.factors }; });
}

function agentPositionSummary() {
  return Object.values(activePositions).map((p:any) => {
    const m:any = latestMetricsPerCoin[p.pair] || {};
    const current = Number(m.currentPrice || p.entryPrice);
    const pnl = p.type === 'long' ? (current-p.entryPrice)*p.amount : (p.entryPrice-current)*p.amount;
    const margin = (p.entryPrice*p.amount)/Math.max(1,p.leverage);
    const roe = margin > 0 ? pnl/margin*100 : 0;
    return { symbol:p.pair, side:p.type, entryPrice:p.entryPrice, currentPrice:current, pnlUSD:pnl, roePct:roe, openedAt:p.openDate, peakNetPnl:p.peakNetPnl || 0 };
  });
}

async function executeAgentCommand(commandText:string) {
  const cmd = parseAgentCommand(commandText);
  const governance = parseGovernanceCommand(commandText);
  if (governance.requestType === 'SET_SECURITY' && governance.recognized && Object.keys(governance.changes).length) {
    const applied = applyAIGovernanceFromText(commandText, 'user');
    persistAIChat({at:Date.now(),role:'user',type:'governance',text:commandText,result:applied});
    return { ok:true, intent:'SET_RISK', governance:applied.state, changed:applied.changed, blocked:applied.blocked, message: `${governance.summary || 'AI güvenlik politikası güncellendi.'}\n${governanceAdviceText(applied.state)}${applied.blocked.length ? `\nDeğiştirilemeyen kritik sınırlar: ${applied.blocked.join('; ')}` : ''}` };
  }
  if (cmd.intent === 'SET_DIRECTIVE' || cmd.intent === 'OPEN_POSITION' || cmd.intent === 'SCAN' || cmd.intent === 'ANALYZE_COIN') applyUserDirective(commandText);

  if (cmd.intent === 'SET_DIRECTIVE') return { ok:true, intent:cmd.intent, directive:agentDirective, message:`Direktif aktif: ${describeDirective(agentDirective)}` };
  if (cmd.intent === 'CHAT') {
    const news = await fetchPublicAINews();
    const recommendation = aiRecommendationSummary();
    const context: AIContext = { ...buildAIContext(news), recommendation };
    const llm = await askLLM(commandText, context);
    persistAIChat({at:Date.now(),role:'user',text:commandText,type:'chat',response:llm.content,provider:llm.provider,model:llm.model});
    return { ok:true, intent:cmd.intent, content:llm.content, provider:llm.provider, model:llm.model, recommendation, cortex:cognitiveUniverseSnapshot };
  }
  if (cmd.intent === 'STATUS') return { ok:true, intent:cmd.intent, directive:agentDirective, positions:agentPositionSummary(), engine:{botState,authenticated:isExchangeAuthenticated,mode:engineMode,autoPilot:aiAutoPilotEnabled}, cortex:cognitiveUniverseSnapshot };
  if (cmd.intent === 'START_ENGINE') { startTradingEngine(); return {ok:true,intent:cmd.intent,message:'Ticaret motoru başlatıldı.',botState}; }
  if (cmd.intent === 'STOP_ENGINE') { await stopTradingEngine(); return {ok:true,intent:cmd.intent,message:'Ticaret motoru durduruldu ve açık pozisyonlar kapatıldı.',botState}; }
  if (cmd.intent === 'AUTOPILOT_ON' || cmd.intent === 'AUTOPILOT_OFF') {
    aiAutoPilotEnabled = cmd.intent === 'AUTOPILOT_ON';
    if (autonomousAILoop) { clearInterval(autonomousAILoop); autonomousAILoop = null; }
    if (aiAutoPilotEnabled) { await refreshAutonomousAI(); autonomousAILoop = setInterval(() => refreshAutonomousAI().catch(()=>{}), 12000); }
    else autonomousAICandidates = {};
    return {ok:true,intent:cmd.intent,autoPilot:aiAutoPilotEnabled};
  }
  if (cmd.intent === 'CLOSE_ALL') {
    if (!isExchangeAuthenticated) return {ok:false,intent:cmd.intent,message:'Gerçek Binance bağlantısı yok; güvenlik nedeniyle emir gönderilmedi.'};
    const results=[];
    for (const symbol of Object.keys(activePositions)) results.push({symbol, ok:await executeExit(symbol,'AI komutu: tüm pozisyonları kapat',Number(latestMetricsPerCoin[symbol]?.currentPrice || activePositions[symbol].entryPrice))});
    return {ok:true,intent:cmd.intent,results};
  }
  if (cmd.intent === 'CLOSE_POSITION') {
    const symbols = cmd.symbol ? [cmd.symbol] : Object.keys(activePositions);
    if (symbols.length !== 1) return {ok:false,intent:cmd.intent,message:'Kapatılacak tek bir sembol belirt: ör. BTC pozisyonunu kapat.'};
    const symbol = symbols[0];
    if (!activePositions[symbol]) return {ok:false,intent:cmd.intent,message:`${symbol} için açık pozisyon bulunamadı.`};
    const ok = await executeExit(symbol,'AI komutu: pozisyon kapat',Number(latestMetricsPerCoin[symbol]?.currentPrice || activePositions[symbol].entryPrice));
    return {ok,intent:cmd.intent,symbol};
  }
  if (cmd.intent === 'ANALYZE_COIN') {
    const coin = getAgentMarketUniverse().find((x:any)=>x.symbol===cmd.symbol);
    if (!coin) return {ok:false,intent:cmd.intent,message:'Coin için yeterli canlı veri yok.'};
    const long = scoreCoinSafe(coin,'long'), short=scoreCoinSafe(coin,'short');
    return {ok:true,intent:cmd.intent,symbol:coin.symbol,analysis:{long,short,best:long.score>=short.score?long:short}};
  }
  if (cmd.intent === 'SCAN' || cmd.intent === 'OPEN_POSITION') {
    const rankings = rankAgentMarkets();
    if (!rankings.length) return {ok:false,intent:cmd.intent,message:'Henüz yeterli canlı market verisi yok.'};
    const candidates = cmd.symbol ? rankings.filter((x:any)=>x.symbol===cmd.symbol) : rankings;
    const selected = candidates.find((x:any)=>x.action==='ENTER_NOW') || candidates[0];
    if (cmd.intent === 'SCAN') return {ok:true,intent:cmd.intent,selected,top:rankings.slice(0,12)};
    if (!selected || selected.action !== 'ENTER_NOW') return {ok:false,intent:cmd.intent,selected,message:`AI şu anda giriş onayı vermiyor. En iyi aday ${selected?.symbol || 'yok'} | olasılık %${Number((selected?.probability||0)*100).toFixed(1)} | güven %${Number(selected?.confidence||0).toFixed(0)}.`};
    if (!agentDirective.allowEntries) return {ok:false,intent:cmd.intent,selected,message:'AI girişleri kapalı. Önce giriş iznini etkinleştir.'};
    if (!exchange || !isExchangeAuthenticated) return {ok:false,intent:cmd.intent,selected,message:'Binance Futures kimlik doğrulaması yok; gerçek emir gönderilmedi.'};
    if (Object.keys(activePositions).length >= Math.min(maxOpenTrades, agentDirective.maxPositions)) return {ok:false,intent:cmd.intent,selected,message:'Maksimum açık pozisyon sayısına ulaşıldı.'};
    const symbol=selected.symbol, side=selected.side, price=Number(latestMetricsPerCoin[symbol]?.currentPrice || 0);
    if (!(price>0)) return {ok:false,intent:cmd.intent,message:'Güncel fiyat yok.'};
    const oldStake = activeStakeAmount, oldLeverage = targetLeverage;
    if (Number.isFinite(cmd.amountUSD) && (cmd.amountUSD as number) > 0) activeStakeAmount = Math.max(1, Math.min(1000000, Number(cmd.amountUSD)));
    if (Number.isFinite(cmd.leverage) && (cmd.leverage as number) > 0) targetLeverage = Math.max(1, Math.min(aiGovernance.maxLeverage, Math.round(Number(cmd.leverage))));
    try {
      const ok=await executeEntry(symbol, side, price);
      return {ok,intent:cmd.intent,selected,executed:{symbol,side,price,stakeAmount:activeStakeAmount,leverage:targetLeverage}};
    } finally {
      activeStakeAmount = oldStake;
      targetLeverage = oldLeverage;
    }
  }

  const context = buildAIContext(await fetchPublicAINews());
  const rankings = rankAgentMarkets().slice(0,12);
  const recommendation = aiRecommendationSummary();
  const isAdvice = parseGovernanceCommand(commandText).requestType === 'ADVICE';
  const enriched = { ...context, directive:{...context.directive, agent:agentDirective, governance:aiGovernance}, agentRanking:rankings, recommendation, chatHistory:aiChatJournal.slice(-12) };
  const result = await askLLM(commandText, enriched as any);
  const fallbackAdvice = isAdvice ? `\n\nCanlı motor tavsiyesi: ${recommendation.summary}\n${recommendation.recommendations.map((x:any)=>`${x.symbol} ${x.side.toUpperCase()} | ${x.action} | olasılık %${x.probability} | güven %${x.confidence} | R/R ${x.riskReward}`).join('\n')}` : '';
  persistAIChat({at:Date.now(),role:'chat',text:commandText,answer:result.content,advice:isAdvice,recommendation});
  return { ...result, content:`${result.content}${fallbackAdvice}`, ok:true, intent:isAdvice?'ADVICE':'CHAT', directive:agentDirective, governance:aiGovernance, rankings, recommendation };
}

function scoreCoinSafe(coin:any, side:AgentSide){
  const rankings = rankMarket([coin] as any, agentDirective, (m:any,s:AgentSide)=>adaptiveAI.predict(extractAIFeatures(m,Number(m.adaptiveUrgency??50),s)));
  return rankings.find((x:any)=>x.side===side) || rankings[0];
}


async function refreshAutonomousAI() {
  if (!aiAutoPilotEnabled || autonomousAI_busy_guard()) return;
  autonomousAIBusy = true;
  try {
    const rankings = rankAgentMarkets();
    const next: typeof autonomousAICandidates = {};
    for (const d of rankings.slice(0, 16)) {
      if (d.action !== 'ENTER_NOW' || d.confidence < agentDirective.minConfidence) continue;
      next[d.symbol] = { side:d.side, confidence:d.confidence, reason:[...d.reasons, ...d.warnings.map(x=>`UYARI: ${x}`)].slice(0,6).join(' | '), updatedAt:Date.now() };
    }
    autonomousAICandidates = next;
    if (Object.keys(next).length) addEngineLog('INFO', `[AI OTONOM V6] ${Object.keys(next).length} aday deterministik agent tarafından onaylandı | ${describeDirective(agentDirective)}`);

    // Otonom giriş yalnızca kullanıcı modu açtıysa, motor çalışıyorsa ve gerçek API bağlantısı doğrulanmışsa devreye girer.
    // Ajanın kendi sert veri/risk kapıları burada da korunur.
    if (exchange && isExchangeAuthenticated && agentDirective.allowEntries && Object.keys(activePositions).length < Math.min(maxOpenTrades, agentDirective.maxPositions)) {
      const best = rankings.find((d:any)=>d.action==='ENTER_NOW' && d.confidence >= agentDirective.minConfidence);
      if (best && !activePositions[best.symbol] && !pendingEntries.has(best.symbol)) {
        const price = Number(latestMetricsPerCoin[best.symbol]?.currentPrice || 0);
        if (price > 0) {
          const ok = await executeEntry(best.symbol, best.side, price);
          if (ok) addEngineLog('TRADE', `[AI OTONOM GİRİŞ] ${best.symbol} ${best.side.toUpperCase()} | Agent %${best.score.toFixed(1)} | Olasılık %${(best.probability*100).toFixed(1)} | Güven %${best.confidence.toFixed(1)}`);
        }
      }
    }

    if (getLLMConfig().provider !== 'none') {
      try {
        const context:AIContext = { ...buildAIContext(await fetchPublicAINews()), coins:getAgentMarketUniverse().slice(0,120) as any };
        const r=await askLLM('Canlı adayları özetle; karar verme, sadece riskler ve dikkat edilmesi gereken veri çatışmalarını kısa maddeler halinde bildir.',context);
        if (r.content) addEngineLog('INFO', `[AI LLM] ${r.content.replace(/\s+/g,' ').slice(0,500)}`);
      } catch {}
    }
  } catch (e:any) {
    addEngineLog('WARN', `[AI OTONOM V6] Tarama kullanılamadı: ${e?.message || e}`);
  } finally { autonomousAI_busy_release(); }
}
function autonomousAI_busy_guard(){ return autonomousAIBusy; }
function autonomousAI_busy_release(){ autonomousAIBusy = false; }
...
app.get("/api/v1/ai/cortex", (req,res)=>{
  const top=Math.min(240,Math.max(1,Number(req.query?.top||80)));
  res.json({ok:true,version:cognitiveCore.version,lastRunAt:cognitiveLastRunAt,state:cognitiveUniverseSnapshot,decisions:cognitiveTopDecisions.slice(0,top),universeCount:allFuturesMarketsList.length,liveTickerCount:latestTickersCache.length,generatedAt:Date.now()});
});

app.get("/api/v1/ai/universe", (req,res)=>{
  const top=Math.min(1000,Math.max(1,Number(req.query?.top||300)));
  const rows=getAgentMarketUniverse().sort((a:any,b:any)=>Number(b.volume_24h_usdt||0)-Number(a.volume_24h_usdt||0));
  res.json({ok:true,count:rows.length,markets:rows.slice(0,top),dataPlane:{allMarketTickers:latestTickersCache.length,trackedDepth:getActiveScanSymbols().length,openPositions:Object.keys(activePositions).length},updatedAt:Date.now()});
});

app.get("/api/v1/ai/portfolio", (req,res)=>{
  const positions=agentPositionSummary();
  const reviewed=positions.map((p:any)=>{ const c=getAgentMarketUniverse().find((x:any)=>x.symbol===p.symbol); return c?{...p,cognitive:cognitiveCore.positionReview(p,c,getAgentMarketUniverse() as CognitiveCoin[],{...DEFAULT_COGNITIVE_POLICY,minConfidence:Number(aiGovernance.minConfidence||72),minProbability:Number(aiGovernance.minProbability||.68),maxUncertainty:Number(aiGovernance.maxUncertainty||.36),minRiskReward:Number(aiGovernance.minRiskReward||1.35)},Date.now())}:p; });
  const long=positions.filter((p:any)=>p.side==='long').reduce((s:any,p:any)=>s+Math.abs(Number(p.pnlUSD||0)),0);
  const short=positions.filter((p:any)=>p.side==='short').reduce((s:any,p:any)=>s+Math.abs(Number(p.pnlUSD||0)),0);
  res.json({ok:true,positions:reviewed,exposure:{longPnlBasis:long,shortPnlBasis:short,total:long+short},generatedAt:Date.now()});
});

app.get("/api/v1/ai/health", (req, res) => {
  const wsReady = Boolean(binanceWsClient && (binanceWsClient as any).readyState === 1);
  res.json({
    ok: true,
    state: aiAutoPilotEnabled ? 'autonomous' : 'standby',
    authenticated: isExchangeAuthenticated,
    engineRunning: botState === 'running',
    websocket: wsReady,
    liveTickerCount: latestTickersCache.length,
    universeCount: allFuturesMarketsList.length,
    positions: Object.keys(activePositions).length,
    lastCortexRunAt: cognitiveLastRunAt,
    lastAgentScanAt: agentLastScanAt,
    autoPilot: aiAutoPilotEnabled,
    hint: !isExchangeAuthenticated
      ? 'Binance Futures API bağlantısı yapılınca emir yürütme etkinleşir; analiz ve tarama devam edebilir.'
      : (aiAutoPilotEnabled && botState !== 'running' ? 'Otonom tarama aktif. Açık emir yetkisi API bağlantısı ve AI güvenlik kapılarıyla korunur.' : 'AI veri ve karar motoru hazır.')
  });
});

app.get("/api/v1/ai/context", async (req, res) => {
  const news = await fetchPublicAINews();
  const context = buildAIContext(news);
  res.json({
    coins: context.coins,
    news,
    directive: { ...aiDirective, governance: aiGovernance },
    autoPilot: aiAutoPilotEnabled,
    ai: { version: adaptiveAI.state.version, features: aiFeatureNames().length, samples: aiJournal.length, trainedSamples: adaptiveAI.state.trainedSamples, threshold: adaptiveAI.state.threshold, agentVersion: 6, cognitiveVersion: cognitiveCore.version, cognitiveUniverse: cognitiveUniverseSnapshot, cognitiveLastRunAt: cognitiveLastRunAt, agentDirective, governance: aiGovernance, hardSafety: HARD_SAFETY, governanceSummary: governanceAdviceText(aiGovernance), chatJournalSize: aiChatJournal.length, agentJournalSize: agentDecisionJournal.length, lastAgentScanAt: agentLastScanAt },
    llm: { ...getLLMConfig(), apiKey: undefined, available: getLLMConfig().provider !== 'none', openPositions: Object.keys(activePositions).length, autonomousCandidates: autonomousAICandidates }
  });
});

app.post("/api/v1/ai/chat", async (req, res) => {
  const message = String(req.body?.message || '').trim();
  if (!message) return res.status(400).json({ ok: false, message: 'Mesaj boş olamaz.' });
  try {
    const result = await executeAgentCommand(message);
    res.json({ ...result, autoPilot: aiAutoPilotEnabled, directive: agentDirective, universeCount: allFuturesMarketsList.length });
  } catch (e:any) {
...
app.post("/api/v1/ai/command", async (req, res) => {
  const command = String(req.body?.command || req.body?.message || '').trim();
  if (!command) return res.status(400).json({ ok:false, message:'Komut boş olamaz.' });
  try { res.json(await executeAgentCommand(command)); }
  catch (e:any) { res.status(500).json({ok:false,message:e?.message || String(e)}); }
});

app.get("/api/v1/ai/ranking", (req,res)=>{
  const top=Math.min(100,Math.max(1,Number(req.query?.top||30)));
  res.json({ok:true, directive:agentDirective, generatedAt:Date.now(), rankings:rankAgentMarkets().slice(0,top)});
});

app.get("/api/v1/ai/positions", (req,res)=>{
  const universe=getAgentMarketUniverse();
  const rows=agentPositionSummary().map((p:any)=>{
    const m=latestMetricsPerCoin[p.symbol]||{};
    const d=analyzePosition(p,m,agentDirective);
    const c=cognitiveCore.positionReview(p as CognitivePosition, (m as any) as CognitiveCoin, universe as CognitiveCoin[], { ...DEFAULT_COGNITIVE_POLICY, minConfidence:Number(aiGovernance.minConfidence||72), minProbability:Number(aiGovernance.minProbability||.68), maxUncertainty:Number(aiGovernance.maxUncertainty||.36), minRiskReward:Number(aiGovernance.minRiskReward||1.35)}, Date.now());
    return {...p,agent:d,cognitive:c};
  });
  res.json({ok:true,rows});
});

app.get("/api/v1/ai/market/:symbol", (req,res)=>{
  const symbol=String(req.params.symbol||'').toUpperCase().includes('/') ? String(req.params.symbol).toUpperCase() : `${String(req.params.symbol).toUpperCase()}/USDT`;
  const coin=getAgentMarketUniverse().find((x:any)=>x.symbol===symbol);
  if(!coin) return res.status(404).json({ok:false,message:'Coin bulunamadı veya canlı verisi yok.'});
  res.json({ok:true,coin,long:scoreCoinSafe(coin,'long'),short:scoreCoinSafe(coin,'short'),directive:agentDirective});
});

app.get("/api/v1/ai/governance", (req,res)=>{
  res.json({ok:true,governance:aiGovernance,hardSafety:HARD_SAFETY,summary:governanceAdviceText(aiGovernance)});
});

app.post("/api/v1/ai/governance", (req,res)=>{
  const result=sanitizeGovernance(req.body||{}, aiGovernance, 'user', 'AI güvenlik paneli');
  aiGovernance=result.state;
  agentDirective={...agentDirective,allowEntries:aiGovernance.allowEntries,autoManagePositions:aiGovernance.autoManagePositions,minConfidence:aiGovernance.minConfidence,minProbability:aiGovernance.minProbability,maxUncertainty:aiGovernance.maxUncertainty,minRiskReward:aiGovernance.minRiskReward,maxPositions:aiGovernance.maxPositions,riskPerTradePct:aiGovernance.riskPerTradePct,allowScaleIn:aiGovernance.allowScaleIn};
  activeStopLossPct=aiGovernance.stopLossPct; activeStakeAmount=aiGovernance.stakeAmountUSDT; maxOpenTrades=aiGovernance.maxPositions; targetLeverage=Math.min(targetLeverage,aiGovernance.maxLeverage);
  persistAIGovernance(); persistAIDirective();
  addEngineLog('SYSTEM',`[AI GOVERNANCE] Panel değişikliği | ${governanceAdviceText(aiGovernance)}`);
  res.json({ok:true,...result,governance:aiGovernance,hardSafety:HARD_SAFETY,summary:governanceAdviceText(aiGovernance)});
});

app.get("/api/v1/ai/advice", async (req,res)=>{
  const recommendation=aiRecommendationSummary();
  let narrative='';
  try {
    const news=await fetchPublicAINews();
    const llm=await askLLM('Canlı piyasa için bana tavsiye ver: en güçlü fırsatlar, kaçınmam gereken durumlar, açık pozisyon riskleri ve bugün özellikle izlemem gereken değişkenler.', { ...buildAIContext(news), recommendation });
    narrative=llm.content;
  } catch {}
  res.json({ok:true,recommendation,narrative,governance:aiGovernance,cortex:{version:cognitiveCore.version,state:cognitiveUniverseSnapshot,top:cognitiveTopDecisions.slice(0,20)},generatedAt:Date.now()});
});

app.post("/api/v1/ai/directive/reset", (req,res)=>{
  agentDirective={...DEFAULT_AGENT_DIRECTIVE,updatedAt:Date.now()};
  aiDirective={};
  persistAIDirective();
  res.json({ok:true,directive:agentDirective});
});

app.post("/api/v1/ai/autopilot", async (req, res) => {
  const enabled = Boolean(req.body?.enabled);
  aiAutoPilotEnabled = enabled;
  if (autonomousAILoop) { clearInterval(autonomousAILoop); autonomousAILoop = null; }
  if (aiAutoPilotEnabled) {
    // Activate immediately. The first scan is intentionally non-blocking so a slow Binance/RSS request
    // cannot make the UI appear stuck or return a false failure. Actual order placement still requires
    // authentication and the normal AI risk gates.
    void refreshAutonomousAI().catch((e:any) => addEngineLog('WARN', `[AI OTONOM] İlk tarama hatası: ${e?.message || e}`));
    autonomousAILoop = setInterval(() => { refreshAutonomousAI().catch(() => {}); }, 15000);
  } else {
    autonomousAICandidates = {};
  }
  addEngineLog('SYSTEM', `[AI] Otonom giriş modu ${aiAutoPilotEnabled ? 'AKTİF' : 'KAPALI'}.`);
  res.json({ ok: true, enabled: aiAutoPilotEnabled, candidates: Object.keys(autonomousAICandidates).length, engineRunning: botState === 'running', authenticated: isExchangeAuthenticated, message: aiAutoPilotEnabled && botState !== 'running' ? 'Otonom tarama aktif; sürekli otomatik giriş için ticaret motoru başlatılabilir.' : undefined });
});
...
app.get("/api/v1/ai/status", (req, res) => {
  res.json({
    ...
    featureCount: aiFeatureNames().length,
    version: adaptiveAI.state.version,
    updatedAt: adaptiveAI.state.updatedAt,
    mode: aiJournal.length >= 30 ? "ready" : "learning",
...
app.get("/api/v1/live-tickers", (req, res) => {
  const results = allFuturesMarketsList.map(sym => {
    ...
    return {
      ...
      bestBid: Number(m?.bestBid || 0), bestAsk: Number(m?.bestAsk || 0),
      microSpreadPct: Number(m?.microSpreadPct || 0),
      cognitiveScore: Number((cognitiveTopDecisions.find((d:any)=>d.symbol===sym)?.score)||0),
      cognitiveAction: cognitiveTopDecisions.find((d:any)=>d.symbol===sym)?.action || '—',
      updated_at: Number(m?.updatedAt || Date.now())
    };
  });
...
app.get("/api/v1/universe", (req, res) => {
  ...
  res.json({
    ...
    live_stream_count: latestTickersCache.length,
